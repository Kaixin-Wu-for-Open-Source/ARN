...
from fairseq.modules.refine_attention import RefineAttention_Merge
...
_MODEL_NAME_ = "arn_transformer"


@register_model(_MODEL_NAME_)
class RanTransformerModel(FairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        decoder = RanTransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        return RanTransformerModel(encoder, decoder)
...
class ArnTransformerDecoder(FairseqIncrementalDecoder):
    """Transformer decoder."""
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=False):
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([TransformerDecoderLayer(args)] + 
                [ArnTransformerDecoderLayer(args) for i in range(2)] + 
    ...
    def forward(self, prev_output_tokens, encoder_out, cache=None, time_step=None, incremental_state=None):
        # embed positions
        ...
        # decoder layers
        slf_attn_memory, cross_attn_memory = None, None
        cross_attn, self_attn, final_attn= None, None, None
        enc_slf_attn_weights, enc_slf_attn_memory = None, None
        for layer_id, layer in enumerate(self.layers):
            x, self_attn_tmp, attn, slf_attn_memory, cross_attn_memory, enc_slf_attn_weights_tmp, enc_slf_attn_memory = layer(
                x,
            ...
            if attn is not None:
                final_attn = attn 
                cross_attn = attn
            if self_attn_tmp is not None:
                self_attn = self_attn_tmp
                enc_slf_attn_weights = enc_slf_attn_weights_tmp
...
class ArnTransformerDecoderLayer(nn.Module):
    """Decoder layer block."""
    ...
    def __init__(self, args):
        ...
        self.refine_attention_merge = RefineAttention_Merge(
    ...
    def forward(self, x, encoder_out, encoder_padding_mask, incremental_state, slf_attn_memory, cross_attn_memory, self_attn, cross_attn, enc_slf_attn_weights, enc_slf_attn_memory, cache):
        residual = x
        x = self.maybe_layer_norm(0, x, before=True)

        self_attn_tmp, attn = None, None
        cur_slf_attn_memory, cur_cross_attn_memory = None, None
        x, new_enc_slf_attn_memory = self.refine_attention_merge(x, cross_attn_memory, 
                cross_attn, encoder_out, slf_attn_memory, self_attn, enc_slf_attn_weights, enc_slf_attn_memory, incremental_state)
        ...
        return x, self_attn_tmp, attn, cur_slf_attn_memory, cur_cross_attn_memory, None, new_enc_slf_attn_memory
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_out, encoder_padding_mask, incremental_state, slf_attn_memory, cross_attn_memory, self_attn, cross_attn, enc_slf_attn_weights, enc_slf_attn_memory, cache):
        ...
        x, self_attn  = self.self_attn(
                ...
                incremental_state=incremental_state,
                need_weights=True,  ## before is False
        )
        slf_attn_memory = x
        ...
        new_enc_slf_attn_memory = cross_attn_memory + slf_attn_memory
        new_enc_slf_attn_weights = torch.cat((attn, self_attn), dim=-1)
        return x, self_attn, attn, slf_attn_memory, cross_attn_memory, new_enc_slf_attn_weights, new_enc_slf_attn_memory
